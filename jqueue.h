...
private:
  ...
  //--------------------------------------------------------
  //helper for dump
  void dumpHelp(Node* ptr) const{
    if (ptr == nullptr){
      return;
    }
    std::cout << "(";
    dumpHelp(ptr->_left);
    std::cout << priority(ptr->_job);
    dumpHelp(ptr->_right);
    std::cout << ")";
  }
  //--------------------------------------------------------
  ...
  //--------------------------------------------------------
  //helper for removing top of heap
  void removeMax(){
    if (_heap != nullptr){
      Node* left = _heap->_left;
      Node* right = _heap->_right;
      delete _heap;
      _heap = mergeHelper(left, right);
    }
  }
  //--------------------------------------------------------
  ...
  Node* copyHelper(Node* ptr){
    ...
    Node* copy = new Node (ptr->_job);
    copy->_left = copyHelper(ptr->_left);
    copy->_right = copyHelper(ptr->_right);
    return copy;
  }
  //--------------------------------------------------------
  //helper for merge
  Node* mergeHelper(Node* ptr1, Node* ptr2){
    //if one skewHeap is empty, return the other
    if (ptr1 == nullptr or ptr2 == nullptr){
      if (ptr1 == nullptr){
        return ptr2;
      }
      else{
    ...
    if(priority(ptr1->_job) < priority(ptr2->_job)){
      std::swap(ptr1, ptr2);
    }
    std::swap(ptr1->_left, ptr1->_right);
    ptr1->_left = mergeHelper(ptr1->_left, ptr2);
    return ptr1;
  }
  //--------------------------------------------------------
  //helper for printJobQueue
  void printPreOrder(Node* ptr) const{
    if (ptr == nullptr){
      return;
    }
    std::cout << *ptr << std::endl;
    printPreOrder(ptr->_left);
    printPreOrder(ptr->_right);
  }  
  //--------------------------------------------------------
